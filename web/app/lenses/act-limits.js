...
(() => {
  ...
  async function renderSafeChange(){
    ...
    $('#evidenceStage').innerHTML=question()+
      ...
      band(2,'Safety gate · fresh preview',`<div id="actionPreview">${empty('A fresh preview will show dependencies, consequences, expiration, exact confirmation phrase, and one-time code.')}</div>`,'Preview expiry is enforced by the server. The UI countdown is an additional visibility aid only.')+

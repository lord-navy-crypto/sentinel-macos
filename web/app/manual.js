...
(() => {
  ...
  function article(topic, index) {
    return `<article class="manual-article" id="manual-${esc(topic.id)}" data-manual-article="${esc(topic.id)}" data-manual-search="${esc([topic.title,topic.kicker,topic.summary,...topic.paragraphs,...topic.lookFor].join(' ').toLowerCase())}">
      ...
      <div class="manual-warning"><b>注意</b><span>${esc(topic.caution)}</span></div>
    </article>`;
  }
  ...
  function renderManual() {
    ...
    stage.innerHTML = question('<button type="button" class="s24-action" data-manual-target="first-five-minutes">新手 5 分钟路线</button><button type="button" class="s24-action" data-manual-target="easy-vs-full">Easy vs Full Scan</button>') + `
      <section class="manual-intro">
        <div><span>USER MANUAL / 使用手册</span><h2>从“我完全不知道 Sentinel 是什么”开始，讲到每个主要功能怎么用。</h2><p>这是一份应用内的长篇手册。左边是可点击目录，右边可以一直向下滚动。每个功能都按“它是什么 → 什么时候用 → 怎么用 → 看什么 → 注意什么”来解释。带 <b>Open feature</b> 的章节可以直接跳到真实功能。</p></div>
        <div class="manual-intro-stats"><b>${allTopics.length}</b><span>详细章节</span><b>${Object.keys(S.LENSES).length}</b><span>Lens / 页面</span></div>
      </section>
      <section class="manual-searchbar"><label for="manualSearch"><span>搜索手册</span><input id="manualSearch" type="search" placeholder="例如：Full Scan、网络、启动项、Safe Change、评分…" autocomplete="off"></label><small id="manualSearchState">显示全部 ${allTopics.length} 个章节</small></section>
      <div class="manual-layout">
        <aside class="manual-toc" aria-label="User Manual table of contents"><div class="manual-toc-title"><span>CONTENTS</span><b>点击章节直接跳转</b></div>${toc()}</aside>

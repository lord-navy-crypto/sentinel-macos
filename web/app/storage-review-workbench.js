...
(() => {
  ...
  function task(label,detail,key){
    if(S.TaskCenter?.create)return S.TaskCenter.create(label,{detail,indeterminate:true,kind:'storage-review',source:TASK_SOURCE,dedupeKey:key});
    return '';
  }
  ...
  function beginReview(key,label,detail,retry){
    const existing=activeReviews.get(key);
    const existingTask=existing&&S.TaskCenter?.tasks?.get(existing);
    if(existingTask?.status==='running'){
      S.TaskCenter.update?.(existing,{detail:'Already running · identical submission ignored'});
    ...
    if(existing)activeReviews.delete(key);
    const id=task(label,detail,key);
    if(id){
      activeReviews.set(key,id);
      if(retry)S.TaskCenter?.setRetry?.(id,'Retry',retryAction(retry));
    }
    return {id,duplicate:false};
  }

  function releaseReview(key,id){if(activeReviews.get(key)===id)activeReviews.delete(key);}

  function completeReview(key,id,detail,html){
    done(id,detail);
    if(id&&html)S.TaskCenter?.setResult?.(id,'Review results',()=>{
      const target=output();
      if(!target)return;
      target.innerHTML=html;
      target.scrollIntoView({behavior:'smooth',block:'nearest'});
    });
    releaseReview(key,id);
  }

  function failReview(key,id,error,retry){
    fail(id,error);
    if(id&&retry)S.TaskCenter?.setRetry?.(id,'Retry',retryAction(retry));
    releaseReview(key,id);
  }
  ...
  async function runOld(form){
    ...
    const key=reviewKey('old',path,days,min),retry=()=>runOld(form);
    const started=beginReview(key,'Old File Review','Bounded modification-age review · no last-used or deletion claim',retry);
    ...
    try{
      ...
      completeReview(key,id,`${rows.length} review row(s)`,html);
    }catch(error){failReview(key,id,error,retry);throw error;}
  }

  async function runDownloads(){
    const key=reviewKey('downloads','~/Downloads'),retry=()=>runDownloads();
    const started=beginReview(key,'Downloads Review','Bounded ~/Downloads review · no cleanup or duplicate hashing',retry);
    if(started.duplicate)return;
    const id=started.id;
    try{
      ...
      output().innerHTML=html;
      completeReview(key,id,`${Number(d.regular_files||0)} Downloads file(s) reviewed`,html);
    }catch(error){failReview(key,id,error,retry);throw error;}
  }
  ...
  async function runDuplicates(form){
    ...
    const key=reviewKey('duplicates',path,min),retry=()=>runDuplicates(form);
    const started=beginReview(key,'Duplicate Review','Bounded full-file SHA-256 equality review',retry);
    if(started.duplicate)return;
    const id=started.id;
    try{
      ...
      output().innerHTML=html;
      completeReview(key,id,`${groups.length} exact duplicate group(s)`,html);
    }catch(error){failReview(key,id,error,retry);throw error;}
  }
  ...
  async function runApp(form){
    ...
    const key=reviewKey('app',app),retry=()=>runApp(form);
    const started=beginReview(key,'App Footprint Review','Measuring app bundle and evidence-linked user Library paths',retry);
    if(started.duplicate)return;
    const id=started.id;
    try{
      ...
      output().innerHTML=html;
      completeReview(key,id,`${items.length} evidence-linked item(s)`,html);
    }catch(error){failReview(key,id,error,retry);throw error;}
  }
  ...
  S.StorageReviewWorkbench={marker:MARKER,openInWorkbench,evidenceFrame,activeReviews};

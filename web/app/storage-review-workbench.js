...
(() => {
  ...
  function reviewKey(type,...parts){return [type,...parts.map(value=>String(value??'').trim())].join('|');}
  ...
  function retryAction(run){return ()=>Promise.resolve(run()).catch(error=>notice(error?.message||String(error)));}

  function createTask(label,detail,key){
    if(!S.TaskCenter?.create)return '';
    return S.TaskCenter.create(label,{detail,indeterminate:true,kind:'storage-review',source:TASK_SOURCE,dedupeKey:key});
  }

  async function executeReview({key,label,detail},operation){
    const existing=activeReviews.get(key);
    if(existing){
      if(existing.taskId)S.TaskCenter?.update?.(existing.taskId,{detail:'Already running · identical submission ignored'});
      notice('An identical storage review is already running. Sentinel did not start a second scan.');
      return;
    }

    const state={taskId:createTask(label,detail,key)};
    activeReviews.set(key,state);
    const retry=retryAction(()=>executeReview({key,label,detail},operation));
    if(state.taskId)S.TaskCenter?.setRetry?.(state.taskId,'Retry',retry);
    ...
    try{
      ...
      if(state.taskId){
        S.TaskCenter?.finish?.(state.taskId,result.detail||'Completed');
      ...
      if(state.taskId){
        S.TaskCenter?.fail?.(state.taskId,error?.message||String(error));
  ...
  async function runOld(form){
    ...
    const meta={key:reviewKey('old',path,days,min),label:'Old File Review',detail:'Bounded modification-age review · no last-used or deletion claim'};
    return executeReview(meta,async()=>{
      ...
      return {detail:`${rows.length} review row(s)`,html:evidenceFrame({observed,interpretation:`<p>${esc(d.definition||'Candidates are defined by modification time and size only.')}</p>`,candidates,unknown:`<p>${esc(d.not_established||'Modification age is not last-opened time. Sentinel has not established whether a file is unused, replaceable, or safe to delete.')}</p>`})};
    });
  }

  async function runDownloads(){
    const meta={key:reviewKey('downloads','~/Downloads'),label:'Downloads Review',detail:'Bounded ~/Downloads review · no cleanup or duplicate hashing'};
    return executeReview(meta,async()=>{
      ...
      return {detail:`${Number(d.regular_files||0)} Downloads file(s) reviewed`,html:evidenceFrame({observed,interpretation:`<p>${esc(d.definition||'Observed Downloads files are grouped by modification age, size, and extension-derived type.')}</p><p>Largest/oldest rows are prioritization views only.</p>`,candidates,unknown:`<p>${esc(d.not_established||'Sentinel has not established last-opened time, duplicate identity, user intent, or deletion safety.')}</p>`})};
    });
  }
  ...
  async function runDuplicates(form){
    ...
    const meta={key:reviewKey('duplicates',path,min),label:'Duplicate Review',detail:'Bounded full-file SHA-256 equality review'};
    return executeReview(meta,async()=>{
      ...
      return {detail:`${groups.length} exact duplicate group(s)`,html:evidenceFrame({observed,interpretation:`<p>${esc(d.definition||'Duplicate means full-file SHA-256 equality among files that completed hashing.')}</p><p>Within each returned group, byte-for-byte hash equality is established for the completed candidates.</p>`,candidates,unknown:'<p>Sentinel has not selected a canonical copy, established which path the user wants to keep, determined whether a copy is referenced elsewhere, or established deletion safety.</p>'})};
    });
  }
  ...
  async function runApp(form){
    ...
    const meta={key:reviewKey('app',app),label:'App Footprint Review',detail:'Measuring app bundle and evidence-linked user Library paths'};
    return executeReview(meta,async()=>{
      ...
      return {detail:`${items.length} evidence-linked item(s)`,html:evidenceFrame({observed,interpretation,candidates,unknown:'<p>Association evidence is not ownership proof. Medium-confidence or Group Container paths may be shared. Sentinel has not established that every listed byte belongs exclusively to this app, nor that any path is safe to remove.</p>'})};
    });
  }

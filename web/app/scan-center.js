...
(() => {
  ...
  const {$, state, api, activity, busy, notice, esc, fmt, badge, registerLens} = S;
  ...
  async function readBaselineState() {
    ...
    const retained = [systemRows.length > 0, networkRows.length > 0, storageRows.length > 0, Boolean(quick.behavior_baseline), Boolean(quick.persistence_baseline)];
    return {
  ...
  function scanStages() {
    return [
      ...
      {id: 'storage', label: 'Deep home-storage traversal & hash analysis', run: async () => { const job = await api('/api/storage/jobs', {method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({scope: 'home', min_mb: 20, limit: 1000})}); fullScan.storageJob = job.id; return pollStorageJob(job.id); }},
  ...
  async function startFullScan() {
    ...
    fullScan.stages = scanStages().map(x => ({...x, status: 'pending', detail: ''}));
    ...
    const limited = fullScan.stages.filter(s => s.status === 'limited').length;
  ...
  S.scanCenter = {marker: SCAN_MARKER, startFullScan, cancelFullScan, readBaselineState, capabilityGroups: CAPABILITY_GROUPS};
})();

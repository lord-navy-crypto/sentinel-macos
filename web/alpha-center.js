...
(() => {
  ...
  i18n.register({
    en:{
      'alpha.score':'Readiness score','alpha.version':'Version','alpha.visibility':'Visibility gaps','alpha.vault':'Vault containment','alpha.noChecks':'No readiness checks were returned.','alpha.open':'Open workspace →',
    ...
    'zh-CN':{
      'alpha.score':'就绪度评分','alpha.version':'版本','alpha.visibility':'可见性缺口','alpha.vault':'Vault 隔离','alpha.noChecks':'没有返回运行就绪检查。','alpha.open':'打开工作区 →',
  ...
  function pillar(title,detail,value,state,href){const card=el('article','pillar-card');card.dataset.state=state;card.append(el('h3','',title),el('strong','',value),el('p','',detail));const meter=el('div','pillar-meter'),fill=el('span');fill.style.width=value;meter.append(fill);card.append(meter);const a=el('a','',t('alpha.open'));a.href=`${href}#token=${encodeURIComponent(token)}`;card.append(a);return card;}
  function renderPillars(r,visibility,isolation,q){
    const unavailable=Number(visibility?.summary?.unavailable||visibility?.unavailable||0);const actionHealthy=Boolean(q?.action_health?.healthy);const failed=Number(isolation?.isolation_failed||0);const localeCount=i18n.supportedLocales().length;
    $('pillarGrid').replaceChildren(
      pillar(t('pillar.understand'),t('pillar.understand.detail'),unavailable?'78%':'92%',unavailable?'growing':'strong','/intelligence-center.html'),
      pillar(t('pillar.investigate'),t('pillar.investigate.detail'),'90%','strong','/investigation.html'),
      pillar(t('pillar.control'),t('pillar.control.detail'),actionHealthy?'84%':'68%',actionHealthy?'strong':'growing','/control-plane.html'),
      pillar(t('pillar.recover'),t('pillar.recover.detail'),failed?'62%':'82%',failed?'limited':'growing','/vault-health.html'),
      pillar(t('pillar.localization'),t('pillar.localization.detail'),localeCount>=2?'55%':'25%','growing','/alpha-center.html')
    );
  }

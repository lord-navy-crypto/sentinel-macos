...
(() => {
  ...
  async function pollStorage(){
    if(!state.scanJob)return;try{const j=await api('/api/storage/jobs?id='+encodeURIComponent(state.scanJob));const phase=String(j.phase||'scan'),phasePct=Number(j.phase_percent||0);let detail=`${Number(j.files_visited||0).toLocaleString()} files · ${Number(j.dirs_visited||0).toLocaleString()} folders`;if(j.slow_paths_skipped)detail+=` · ${Number(j.slow_paths_skipped).toLocaleString()} slow paths skipped`;if(j.hash_files_total)detail+=` · hashes ${Number(j.hash_files_done||0)}/${Number(j.hash_files_total||0)}`;if(j.hash_bytes_total)detail+=` · ${bytes(j.hash_bytes_done||0)} / ${bytes(j.hash_bytes_total||0)}`;if(j.current_hash_path)detail+=` · ${j.current_hash_path}`;activity(phase.replaceAll('_',' '),phasePct,detail);const steps=$$('#storagePipeline .s24-step');const idx=phase.includes('hash')?2:phase.includes('report')?3:phase.includes('measure')?1:0;steps.forEach((x,i)=>{x.classList.toggle('active',i===idx);x.classList.toggle('done',i<idx);});if(j.status==='running'){state.scanTimer=setTimeout(pollStorage,500);return;}if(j.status==='failed')throw new Error(j.error||'Storage scan failed');if(j.result)renderStorageResult(j.result,j.status);activity(j.status==='cancelled'?'Cancelled':'Complete',100,j.status==='cancelled'?'Partial measured result preserved when available.':'Building storage report complete.');}catch(e){notice(e.message);activity('Error',0,e.message)}
  }

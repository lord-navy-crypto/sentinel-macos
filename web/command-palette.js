...
(() => {
  ...
  // Main-dashboard entry point. Standalone workspaces simply skip this block.
  const sidebar=document.querySelector('.sidebar-actions');
  if(sidebar&&!document.getElementById('intelligenceCenterShortcut')){
    const shortcut=el('button',{id:'intelligenceCenterShortcut',class:'side-action',type:'button'},'Intelligence Center');
    shortcut.title='Open Evidence Graph 2.0, Incident Intelligence 2.0, Global Timeline, Object Story 2.0, and Visibility';
    shortcut.addEventListener('click',()=>{location.href=destination('/intelligence-center.html')});sidebar.append(shortcut);
  }
  ...
  async function search(){const current=++seq;const q=input.value.trim();if(!q){render([{kind:'navigation',label:'Intelligence Center',href:'/intelligence-center.html'},{kind:'navigation',label:'Evidence Graph 2.0',href:'/intelligence-center.html#graph'},{kind:'navigation',label:'Incident Intelligence 2.0',href:'/intelligence-center.html#incidents'},{kind:'navigation',label:'Global Timeline',href:'/intelligence-center.html#timeline'},{kind:'navigation',label:'Object Story 2.0',href:'/intelligence-center.html#object'},{kind:'navigation',label:'Visibility & Permissions',href:'/intelligence-center.html#visibility'},{kind:'navigation',label:'Network Relationships',href:'/network-relations.html'},{kind:'navigation',label:'Launch & Service Explorer',href:'/launch-services.html'}]);return}try{const r=await fetch('/api/search/command?q='+encodeURIComponent(q),{headers:{'X-Sentinel-Token':token()}});const data=await r.json().catch(()=>({actions:[]}));if(current!==seq)return;if(!r.ok)throw new Error(data.error||`HTTP ${r.status}`);render(data.actions)}catch(e){if(current===seq){results.replaceChildren(el('div',{class:'sentinel-command-empty'},e.message||String(e)))}}}

...
func TestExpandedStructuredParsersAndContinuationTargets(t *testing.T) {
	...
	foundPath := false
	for _, r := range apfs.Records { if r.Path == "/System/Volumes/Data" { foundPath = true } }
	if !foundPath { t.Fatalf("expected APFS mount path continuation evidence") }
	...
	targets := SystemConsoleContinuationTargets(SystemConsoleResult{}, parsed)
	if len(targets) < 2 { t.Fatalf("expected PID and path continuation targets: %#v", targets) }
}
...
func TestSystemEvidenceManagerBoundedAndNoRawOutputField(t *testing.T) {
	...
	rows:=m.list(systemEvidenceHistoryLimit+20)
	if len(rows)!=systemEvidenceHistoryLimit { t.Fatalf("retention=%d want=%d",len(rows),systemEvidenceHistoryLimit) }
	// The retained observation type deliberately has no raw-output field.
...
func TestSystemEvidenceIncidentBridgeUsesRealAbsolutePathsOnly(t *testing.T) {
	...
	controlEphemeral.systemEvidence.add(SystemEvidenceObservation{ID:"x",At:1,ToolID:"gatekeeper-assessment",Target:path,Signals:[]SystemEvidenceSignal{{Code:"gatekeeper_rejected",Severity:"review",IncidentEligible:true,Summary:"review"}}})

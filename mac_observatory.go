...
func parseVMStatObservatory(raw string) map[string]uint64 {
	...
	if i := strings.Index(raw, "page size of "); i >= 0 {
		...
		if len(fields) > 0 {
			if n, err := strconv.ParseUint(fields[0], 10, 64); err == nil { pageSize = n }
		}
	}
...
func parseProcessSample(raw string) (float64, []resourceProcess) {
	...
	for scanner.Scan() {
		...
		pid, err1 := strconv.Atoi(fields[0]); cpu, err2 := strconv.ParseFloat(fields[1], 64); mem, err3 := strconv.ParseFloat(fields[2], 64)
		if err1 != nil || err2 != nil || err3 != nil { continue }
		command := strings.Join(fields[3:], " ")
		total += cpu
		rows = append(rows, resourceProcess{PID: pid, CPU: cpu, Memory: mem, Command: command})
	}
	sort.Slice(rows, func(i, j int) bool { if rows[i].CPU == rows[j].CPU { return rows[i].Memory > rows[j].Memory }; return rows[i].CPU > rows[j].CPU })
...
func parseNetworkCounters(raw string) (uint64, uint64) {
	...
	for scanner.Scan() {
		...
		inTotal += nums[len(nums)-2]; outTotal += nums[len(nums)-1]; seen[iface] = true
	}
...
func (o *resourceObservatory) append(sample resourceSample) resourceSample {
	...
	if len(o.history) > 0 {
		...
		pt, e1 := time.Parse(time.RFC3339Nano, prev.CapturedAt); ct, e2 := time.Parse(time.RFC3339Nano, sample.CapturedAt)
		if e1 == nil && e2 == nil {
			seconds := ct.Sub(pt).Seconds()
			if seconds > 0 {
				if sample.NetworkInBytes >= prev.NetworkInBytes { sample.NetworkInBPS = float64(sample.NetworkInBytes-prev.NetworkInBytes) / seconds }
				if sample.NetworkOutBytes >= prev.NetworkOutBytes { sample.NetworkOutBPS = float64(sample.NetworkOutBytes-prev.NetworkOutBytes) / seconds }
			}
		}
	}
	o.history = append(o.history, sample)
	if len(o.history) > observatoryHistoryLimit { o.history = append([]resourceSample(nil), o.history[len(o.history)-observatoryHistoryLimit:]...) }
	return sample
}
...
type storageGraphNode struct { Name string `json:"name"`; Path string `json:"path"`; Bytes int64 `json:"bytes"`; IsDir bool `json:"is_dir"`; Percent float64 `json:"percent"` }

func (a *app) handleStorageGraph(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodGet { writeJSON(w, http.StatusMethodNotAllowed, map[string]any{"error":"GET required"}); return }
	requested := strings.TrimSpace(r.URL.Query().Get("path")); if requested == "" { requested, _ = os.UserHomeDir() }
	clean, err := filepath.Abs(filepath.Clean(requested)); if err != nil { writeJSON(w, 400, map[string]any{"error":"invalid path"}); return }
	info, err := os.Stat(clean); if err != nil { writeJSON(w, 404, map[string]any{"error":err.Error()}); return }; if !info.IsDir() { writeJSON(w, 400, map[string]any{"error":"Storage Graph expands directories only"}); return }
	limit := 24; if raw := r.URL.Query().Get("limit"); raw != "" { if n, err := strconv.Atoi(raw); err == nil && n >= 6 && n <= 60 { limit = n } }
	...
	nodes := []storageGraphNode{}; parentBytes := int64(0)
	scanner := bufio.NewScanner(strings.NewReader(raw))
	for scanner.Scan() {
		fields := strings.SplitN(scanner.Text(), "\t", 2); if len(fields) != 2 { continue }
		kb, err := strconv.ParseInt(strings.TrimSpace(fields[0]), 10, 64); if err != nil { continue }
		p := strings.TrimSpace(fields[1]); b := kb*1024; if filepath.Clean(p) == clean { parentBytes = b; continue }
		st, statErr := os.Stat(p); nodes = append(nodes, storageGraphNode{Name:filepath.Base(p),Path:p,Bytes:b,IsDir:statErr==nil&&st.IsDir()})
	}
	...
	writeJSON(w,http.StatusOK,map[string]any{"marker":macObservatoryMarker,"path":clean,"name":filepath.Base(clean),"bytes":parentBytes,"children":nodes,"hidden_children":hidden,"limited":limited,"detail":func()string{if limited{return "Some entries could not be measured because of permissions or filesystem boundaries."};return "Measured with a bounded, read-only directory-size query."}()})
}

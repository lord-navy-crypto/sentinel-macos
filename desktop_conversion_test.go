...
func TestDesktopDistributionAssets(t *testing.T) {
	...
	for path, needles := range checks {
		...
		for _, needle := range needles {
			if !strings.Contains(s, needle) { t.Fatalf("%s missing %q", path, needle) }
		}
	}
...
func TestBrowserAndNativeAppViewUseSameSentinel24Product(t *testing.T) {
	...
	for _, needle := range []string{
		...
		"same Sentinel 2.4 Native Frontend",
	} {
	...
	for _, needle := range []string{`X-Sentinel-UI", "2.4-native`,`fs.ReadFile(staticFS, "index.html")`,`_, _ = w.Write(page)`} {
		if !strings.Contains(mainSource, needle) { t.Fatalf("server must serve the 2.4 product source directly; missing %q", needle) }
	}
...
func TestSentinel24UsesOneViewportWithProgressiveContext(t *testing.T) {
	...
	for _, needle := range []string{".s24-shell{position:fixed","grid-template-rows:58px 54px 40px minmax(0,1fr) 32px",".s24-stage{","overflow-y:auto",".s24-context{position:fixed",".s24-activity{"} {
		if !strings.Contains(css, needle) { t.Fatalf("Sentinel 2.4 viewport contract missing %q", needle) }
	}
	if strings.Contains(css, "grid-template-columns:244px minmax(0,1fr)") { t.Fatal("Sentinel 2.4 must not return to the retired 244px dashboard sidebar layout") }
}

...
func TestOneClickCheckIsNativeProductSnapshot(t *testing.T) {
	...
	for _, bad := range []string{"/api/actions/execute", "/api/changes/start", "/api/trust/capture", "sudo ", "rm -"} {
		if strings.Contains(js, bad) {
			t.Fatalf("snapshot/investigation module contains mutation %q", bad)
		}
	}
}
func TestMainRegistersVaultIsolationReadEndpoint(t *testing.T) {
	...
	if !strings.Contains(text, "mux.HandleFunc(\"/api/actions/vault/isolation\"") {
		t.Fatal("Vault isolation route missing")
	}
	if !strings.Contains(text, "a.handleVaultIsolation") {
		t.Fatal("Vault isolation handler missing")
	}
}

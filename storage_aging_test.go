...
func TestBuildStorageAgingReportBucketsAndTrend(t *testing.T){
	...
	if got.FilesConsidered!=4||got.BytesConsidered!=100{t.Fatalf("considered=%+v",got)}
	if got.Buckets[0].Files!=1||got.Buckets[1].Files!=1||got.Buckets[3].Files!=1||got.Buckets[4].Files!=1{t.Fatalf("buckets=%+v",got.Buckets)}
	if len(got.Trend)!=2||got.Trend[1].SnapshotID!="s2"||!got.Trend[1].Partial{t.Fatalf("trend=%+v",got.Trend)}
	if got.OldestLargeFiles[0].Path!="/Users/test/d"{t.Fatalf("oldest=%+v",got.OldestLargeFiles)}
}

func TestBuildStorageAgingReportMissingDataIsExplicit(t *testing.T){
	got:=BuildStorageAgingReport(nil,nil,time.Unix(1,0))
	if len(got.Limitations)==0||got.FilesConsidered!=0{t.Fatalf("missing report=%+v",got)}
}

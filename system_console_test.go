...
func TestSystemConsoleIntegrationContract(t *testing.T) {
	checks := map[string][]string{
		...
		"web/aux-navigation.js":   {"Sentinel 2.7 · AUX", "productHref('status')", "/system-console.html"},
	}
...
func TestSystemConsoleCannotBecomeArbitraryShell(t *testing.T) {
	...
	for _, forbidden := range []string{`exec.Command("sh"`, `exec.Command("bash"`, `exec.Command("zsh"`, `exec.CommandContext(ctx, "sh"`, `exec.CommandContext(ctx, "bash"`, `exec.CommandContext(ctx, "zsh"`, `"/bin/sh"`, `"/bin/bash"`, `"/bin/zsh"`, `"sudo"`} {

	...
	for path, required := range checks {
		...
		for _, forbidden := range []string{"eval(", "new function(", "document.write(", "innerhtml", "sudo "} {
			if strings.Contains(lower, forbidden) {
				t.Fatalf("%s contains forbidden UI pattern %q", path, forbidden)
			}
		}
	}
}
...
func TestExpandedLogRecipesRemainFixedAndBounded(t *testing.T) {
	...
	for _, tool := range systemConsoleToolDefinitions() {
		...
		want[tool.ID] = true
		if tool.Mode != "read_only" || tool.Command != "/usr/bin/log" {
			t.Fatalf("log recipe %s not fixed read-only /usr/bin/log: %#v", tool.ID, tool)
		}
		if tool.TimeoutSeconds <= 0 || tool.TimeoutSeconds > 15 {
			t.Fatalf("log recipe %s has invalid timeout", tool.ID)
		}
		joined := strings.Join(tool.BaseArgs, " ")
		if !strings.Contains(joined, "--last 10m") || !strings.Contains(joined, "--predicate") {
			t.Fatalf("log recipe %s is not bounded/fixed: %q", tool.ID, joined)
		}
	}
	for id, ok := range want {
		if !ok {
			t.Fatalf("missing bounded log recipe %s", id)
		}
	}
}

...
func TestProcessRelationshipExplorerCombinesExistingBoundedEvidence(t *testing.T) {
	requireProcessRelationSourceContains(t, "web/process-relations.html",
		`Process Relationship Explorer`,
		`Parent & Child Processes`,
		`Open Objects & TCP`,
		`Why can this executable start?`,
		`Object Story`,
		`/process-relations.js`,
	)
	requireProcessRelationSourceContains(t, "web/process-relations.js",
		`/api/process/detail?pid=`,
		`/api/object/story?pid=`,
		`process-table`,
		`process-open-files`,
		`/api/security/context?path=`,
		`Continue Investigation on Executable`,
		`Inspect PID`,
		`Child-process correlation is a current bounded process-table snapshot`,
	)
}

func TestProcessRelationshipExplorerIsLinkedFromSystemConsole(t *testing.T) {
	requireProcessRelationSourceContains(t, "web/system-console.html",
		`processRelationsRecipe`,
		`How is this process connected?`,
		`/process-relations.html`,
	)
	requireProcessRelationSourceContains(t, "web/system-console-links.js",
		`processRelationsRecipe`,
		`/process-relations.html#token=`,
	)
}

...
func TestVaultHealthUIUsesRecoveryReadAPIsOnly(t *testing.T){
	...
	all:=string(html)+"\n"+string(js)
	for _,want:=range []string{"/api/actions/health","/api/actions/vault","/api/actions/journal","Post-action observation","Investigate source","Investigate destination"}{if !strings.Contains(all,want){t.Fatalf("Vault Health missing %q",want)}}

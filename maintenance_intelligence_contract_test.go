...
func TestMaintenanceHashRequiresFullFileEquality(t *testing.T) {
	...
	if err := os.WriteFile(b, []byte("same-content"), 0600); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(c, []byte("different---"), 0600); err != nil {
		t.Fatal(err)
	}
	deadline := time.Now().Add(2 * time.Second)
	ha, err := hashFileBounded(a, deadline)
	if err != nil {
		t.Fatal(err)
	}
	hb, err := hashFileBounded(b, deadline)
	if err != nil {
		t.Fatal(err)
	}
	hc, err := hashFileBounded(c, deadline)
	if err != nil {
		t.Fatal(err)
	}
	if ha != hb {
		t.Fatal("equal files must have equal SHA-256")
	}
	if ha == hc {
		t.Fatal("different files must not be labeled duplicate")
	}
}

...
func (a *app) handleControlledGitPull(w http.ResponseWriter, r *http.Request) {
	...
	if a.ephemeral {
		writeJSON(w, http.StatusForbidden, map[string]any{"error": "controlled mutations are disabled in ephemeral mode"})
		return
	}
	...
	if err != nil {
		writeJSON(w, http.StatusConflict, map[string]any{"error": "current Git HEAD could not be recorded before pull"})
		return
	}
	output, err := gitOutput(ctx, preview.TopLevel, "pull", "--ff-only")
	if err != nil {
		writeJSON(w, http.StatusConflict, map[string]any{"error": err.Error(), "output": output, "preview": preview, "before_head": before})
		return
	}
...
func expandDownloadDestination(raw string) (string, string, error) {
	...
	downloadsInfo, err := os.Stat(downloads)
	if err != nil || !downloadsInfo.IsDir() {
		return "", "", errors.New("Downloads folder must already exist")
	}
	resolvedDownloads, err := filepath.EvalSymlinks(downloads)
	if err != nil {
		return "", "", errors.New("Downloads folder could not be resolved")
	}
	...
	if err != nil {
		return "", "", errors.New("destination parent could not be resolved")
	}
	...
	if info, err := os.Lstat(resolvedDestination); err == nil && info != nil {

...
func TestFullScanCenterIsCanonicalProductModule(t *testing.T) {
	...
	for _, retired := range []string{"/app/scan-center.css", "/app/scan-center.js"} {
		if strings.Contains(s, retired) {
			t.Fatalf("canonical product revived retired Scan Center asset %q", retired)
		}
	}
}
...
func TestFullScanCenterUsesRealRetainedEvidenceChain(t *testing.T) {
	...
	for _, want := range []string{
		"Sentinel 2.7 Full Scan Center", "Easy Scan", "Full Scan", "Complete Capability Atlas",
		...
		if !strings.Contains(s, want) {
			t.Fatalf("Full Scan Center missing real evidence contract %q", want)
		}
	}
}
...
func TestFullScanCenterPreservesSafetyAndFreshnessBoundaries(t *testing.T) {
	...
	for _, want := range []string{
		...
		if !strings.Contains(s, want) {
			t.Fatalf("Full Scan safety/freshness boundary missing %q", want)
		}
	}
...
func TestFullScanStartupIsLightweightAndNonBlocking(t *testing.T) {
	...
	for _, want := range []string{
		...
		if !strings.Contains(s, want) {
			t.Fatalf("lightweight Full Scan startup guard missing %q", want)
		}
	}
}
...
func TestCapabilityAtlasCoversAllPrimaryLenses(t *testing.T) {
	...
	for _, lens := range []string{
		...
		if !strings.Contains(s, "'"+lens+"'") {
			t.Fatalf("capability atlas missing lens %q", lens)
		}
	}
}
...
func TestScanCenterVisualSystemIsResponsiveAndWide(t *testing.T) {
	...
	for _, want := range []string{
		...
		if !strings.Contains(s, want) {
			t.Fatalf("Scan Center visual system missing %q", want)
		}
	}
}

...
func TestReasonCodeRegistryValidAndCoversGeneratedCodes(t *testing.T) {
	reg := ReasonCodeRegistry()
	if err := ValidateReasonCodeRegistry(reg); err != nil {
		t.Fatal(err)
	}
	...
	for _, r := range BuildIncidentExplanation(in).ReasonCodes {
		if !ReasonCodeDefined(r.Code) {
			t.Fatalf("generated reason code missing from registry: %q", r.Code)
		}
	}
}

func TestDefaultIncidentRuleRegistryReferencesRegisteredReasons(t *testing.T) {
	reg := DefaultIncidentRuleRegistry()
	if err := ValidateIncidentRuleRegistry(reg); err != nil {
		t.Fatal(err)
	}
	if reg.Version != IncidentRuleRegistryVersion || len(reg.Rules) < 3 {
...
func TestReasonRegistryRejectsDuplicateCode(t *testing.T) {
	reg := ReasonCodeRegistry()
	reg.Definitions = append(reg.Definitions, reg.Definitions[0])
	if err := ValidateReasonCodeRegistry(reg); err == nil {
		t.Fatal("duplicate reason code should fail")
	}
}

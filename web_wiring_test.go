...
func readUIFile(t *testing.T, path string) string { t.Helper(); b, err := os.ReadFile(path); if err != nil { t.Fatal(err) }; return string(b) }
...
func TestSentinelApplicationAPIReferencesHaveRegisteredRoutes(t *testing.T) {
	js := readProductScripts(t); server := readUIFile(t, "main.go")
	re := regexp.MustCompile("[\"'`](/api/[A-Za-z0-9_./-]+)[^\"'`]*[\"'`]"); seen := map[string]bool{}; var paths []string
	for _, match := range re.FindAllStringSubmatch(js, -1) { path := match[1]; if !seen[path] { seen[path]=true; paths=append(paths,path) } }
	sort.Strings(paths); if len(paths) < 25 { t.Fatalf("direct frontend references only %d API routes", len(paths)) }
	for _, path := range paths { if !strings.Contains(server, `mux.HandleFunc("`+path+`"`) && !strings.Contains(server, `mux.Handle("`+path+`"`) { t.Errorf("frontend route %s is not registered", path) } }
}
...
func TestSentinelApplicationUsesAuthenticatedLocalRequests(t *testing.T) {
	...
	for _, want := range []string{"location.hash.slice(1)","X-Sentinel-Token","127.0.0.1","X-Frame-Options","frame-ancestors 'none'","connect-src 'self'"} { if !strings.Contains(all,want) { t.Fatalf("local UI security contract missing %q",want) } }
}

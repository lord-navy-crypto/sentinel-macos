...
func TestGroupInvestigationTimelineCollapsesOnlyMatchingNearbyEvents(t *testing.T){
	...
	if len(groups)!=3{t.Fatalf("groups=%d want=3: %#v",len(groups),groups)}
	if groups[0].Count!=2||groups[0].FirstAt!=100||groups[0].LastAt!=120{t.Fatalf("first group=%+v",groups[0])}
	if len(groups[0].EventIDs)!=2{t.Fatalf("event provenance=%v",groups[0].EventIDs)}
	if groups[1].Count!=1||groups[2].Kind!="rename"{t.Fatalf("nonmatching evidence must stay separate: %#v",groups)}
}
...
func TestGroupInvestigationTimelinePreservesIncidentBoundary(t *testing.T){
	...
	if got:=len(GroupInvestigationTimeline(rows,60,20));got!=2{t.Fatalf("incident-specific events merged, groups=%d",got)}
}

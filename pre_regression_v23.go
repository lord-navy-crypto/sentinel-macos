...
type PreRegressionCheckV23 struct {
	ID string `json:"id"`
...
type PreRegressionReportV23 struct {
	GeneratedAt string `json:"generated_at"`
	Schema int `json:"schema"`
	ReadyForRealRegression bool `json:"ready_for_real_regression"`
	Blockers int `json:"blockers"`
	Checks []PreRegressionCheckV23 `json:"checks"`
	Migration V23MigrationReport `json:"migration"`
	ManualRegressionNext []string `json:"manual_regression_next"`
	Note string `json:"note"`
}

func buildPreRegressionReportV23(a *app) PreRegressionReportV23 {
	r:=PreRegressionReportV23{GeneratedAt:time.Now().UTC().Format(time.RFC3339),Schema:SentinelSchemaV23,ReadyForRealRegression:true,Migration:currentV23MigrationReport(),Note:"This gate validates engineering preparation only. Passing it means the branch is ready to begin real macOS regression testing; it does not certify runtime behavior on a specific Mac or release artifact."}
	add:=func(id,status,detail string,real bool){r.Checks=append(r.Checks,PreRegressionCheckV23{ID:id,Status:status,Detail:detail,RequiresRealMac:real});if status=="blocker"{r.Blockers++;r.ReadyForRealRegression=false}}

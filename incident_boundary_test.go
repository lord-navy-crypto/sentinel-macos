...
func TestIncidentStoryStableAcrossEpisodeWindows(t *testing.T) {
	...
	if !ok {
		t.Fatal("first episode should produce an incident")
	}
	b, ok := incidentFromCluster(anchor, second)
	if !ok {
		t.Fatal("second episode should produce an incident")
	}
	if a.StoryKey != b.StoryKey {
		t.Fatalf("same object must keep stable story key: %q != %q", a.StoryKey, b.StoryKey)
	}
	if a.ID == b.ID {
		t.Fatalf("bounded episodes must retain distinct episode IDs")
	}
}
...
func TestIncidentStoriesSplitByCanonicalAnchor(t *testing.T) {
	...
	if !ok {
		t.Fatal("A should produce incident")
	}
	b, ok := incidentFromCluster("/tmp/B", []IncidentEvidence{{At: 10, Source: "behavior", Kind: "changed", Severity: "review", Path: "/tmp/B", Detail: "b"}})
	if !ok {
		t.Fatal("B should produce incident")
	}
	if a.StoryKey == b.StoryKey {
		t.Fatalf("different canonical objects must not merge into one story")
	}
}
...
func TestNormalizeLoadedIncidentMigratesWindowedStoryKey(t *testing.T) {
	...
	if got.StoryKey != want {
		t.Fatalf("story migration=%q want=%q", got.StoryKey, want)
	}
}
...
func TestIncidentManagerMergesHistoryByStableStory(t *testing.T) {
	...
	if len(m.history) != 1 {
		t.Fatalf("history stories=%d want=1", len(m.history))
	}
	if m.history[0].CreatedAt != 10 || m.history[0].UpdatedAt != cur.UpdatedAt {
		t.Fatalf("merged bounds=%+v", m.history[0])
	}
	if len(m.history[0].Evidence) != 2 {
		t.Fatalf("merged evidence=%d want=2", len(m.history[0].Evidence))
	}
	if m.history[0].ID != "new" {
		t.Fatalf("latest episode ID should be retained, got %q", m.history[0].ID)
	}
}

...
func TestMigrationRegistryCoversLegacyStores(t *testing.T) {
	...
	for _, id := range migrationRegistryIDs() {
		delete(want, id)
	}
	if len(want) != 0 {
		t.Fatalf("missing migration registry stores: %#v", want)
	}
}

func TestMigrateBehaviorBaselineCreatesRollbackCopy(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "behavior-baseline.json")
	legacy := BehaviorSnapshot{Version: 0, CapturedAt: "2026-01-01T00:00:00Z"}
	if err := writePrivateJSON(path, legacy); err != nil {
		t.Fatal(err)
	}
	r := migrateBehaviorBaseline(path)
	...
	if err := strictReadJSON(path, &got); err != nil {
		t.Fatal(err)
	}
	if got.Version != 1 || got.PrivacyNote == "" {
...
func TestMigrationRefusesCorruptPrimaryWithoutOverwrite(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "trust-profile.json")
	before := []byte("{broken")
	if err := os.WriteFile(path, before, 0600); err != nil {
		t.Fatal(err)
	}
	r := migrateTrustProfile(path)
	if r.Status != "error" || r.Changed {
		t.Fatalf("corrupt migration=%+v", r)
	}
	...
	if string(after) != string(before) {
		t.Fatalf("corrupt primary was rewritten")
	}
}

func TestMigrateIncidentHistoryV2ToV3StableStory(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "incident-history.json.gz")
	...
	if err := writePrivateGzipJSON(path, legacy); err != nil {
		t.Fatal(err)
	}
	r := migrateIncidentHistory(path)
	if r.Status != "ok" || !r.Changed {
		t.Fatalf("migration=%+v", r)
	}
	var got struct {
	...
	if err := strictReadGzipJSON(path, &got); err != nil {
		t.Fatal(err)
	}
	if got.Version != incidentHistoryVersion || len(got.Incidents) != 1 {
		t.Fatalf("migrated=%+v", got)
	}
	if got.Incidents[0].StoryKey != entityID("incident-story", p) {
		t.Fatalf("story=%q", got.Incidents[0].StoryKey)
	}
	if len(got.Incidents[0].Evidence) != 2 {
		t.Fatalf("evidence=%d", len(got.Incidents[0].Evidence))
	}
}

func TestEphemeralMigrationDoesNotApplyPersistentWrites(t *testing.T) {
	r := runStateMigrations(true)
	if r.Applied || !r.Healthy || len(r.Results) != 1 || r.Results[0].Status != "skipped" {
		t.Fatalf("ephemeral report=%+v", r)
	}
}

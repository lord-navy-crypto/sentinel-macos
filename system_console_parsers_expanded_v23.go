...
func parseColonFacts(kind, raw string, limit int) ParsedSystemEvidence {
	...
	out.ParsedRows=len(out.Facts)
	return out
}

func parseStatusFact(kind, label, raw string) ParsedSystemEvidence {
	out := ParsedSystemEvidence{Kind:kind}
	value := firstMeaningfulLine(raw)
	state := "unknown"
	lower := strings.ToLower(value)
	switch {
	case strings.Contains(lower,"enabled") || strings.Contains(lower," is on") || strings.Contains(lower,"active"):
		state="ok"
	case strings.Contains(lower,"disabled") || strings.Contains(lower," is off") || strings.Contains(lower,"inactive"):
		state="review"
	}
	appendFact(&out,label,value,state)
	out.ParsedRows=len(out.Facts)
	return out
}

func parseLaunchctlEvidence(raw string) ParsedSystemEvidence {
	out := ParsedSystemEvidence{Kind:"launch_services"}
...
func parseAPFSEvidence(raw string) ParsedSystemEvidence {
	out:=ParsedSystemEvidence{Kind:"apfs_layout"}
	group:="APFS"
	for _,line:=range strings.Split(raw,"\n") {
...
func parsePowerAssertionsEvidence(raw string) ParsedSystemEvidence {
	...
	for _,line:=range strings.Split(raw,"\n") {
		...
		lower:=strings.ToLower(line)
		if strings.Contains(lower,"pid ") || strings.Contains(lower,"preventusersystemsleep") || strings.Contains(lower,"preventuseridl esystemsleep") || strings.Contains(lower,"preventdisplay sleep") {
			pid:=0
...
func parseBoundedLines(kind, group, raw string, limit int) ParsedSystemEvidence {
	out:=ParsedSystemEvidence{Kind:kind}
	for _,line:=range boundedUniqueLines(raw,limit) {
		out.Records=append(out.Records,SystemEvidenceRecord{Kind:kind+"_row",Group:group,Label:line})
	}
	out.ParsedRows=len(out.Records); return out
}
...
func ParseExpandedSystemConsoleEvidence(toolID, raw string) (ParsedSystemEvidence,bool) {
	...
	case "listening-processes":
		p:=ParseOpenFileEvidence(raw); p.Kind="listening_processes"; return p,true
...
func SystemConsoleContinuationTargets(result SystemConsoleResult, parsed ParsedSystemEvidence) []SystemConsoleContinuationTarget {
	out:=[]SystemConsoleContinuationTarget{}

...
func TestInvestigationSessionFindReturnsDetachedCopy(t *testing.T){
	...
	s,ok:=m.find("x");if !ok{t.Fatal("session not found")};s.Branches[0].Path="/mutated";again,_:=m.find("x");if again.Branches[0].Path!="/tmp/A"{t.Fatal("find returned shared branch storage")}
}

...
p = Path('web/app/lenses/act-limits.js')
s = p.read_text()
start = s.index('  async function renderReclaim()')
end = s.index('  async function renderVisibility()')
new = r'''  const SAFE_CHANGE_WORKFLOW_MARKER='Sentinel 2.7 Safe Change Workflow';
...
'''
s = s[:start] + new + s[end:]
# Export marker/helpers for contracts and future UI integration.
s = s.replace("  S.previewSafeAction=previewSafeAction;S.executeSafeAction=executeSafeAction;S.renderSafeChange=renderSafeChange;", "  S.previewSafeAction=previewSafeAction;S.executeSafeAction=executeSafeAction;S.renderSafeChange=renderSafeChange;S.safeChangeWorkflow={marker:SAFE_CHANGE_WORKFLOW_MARKER,previewRemainingMs,previewActionRequest};")
p.write_text(s)

p = Path('web/app/runtime-logs.js')
s = p.read_text()
old='<option value="workbench">workbench</option><option value="storage">storage</option>'
new='<option value="workbench">workbench</option><option value="action">action</option><option value="storage">storage</option>'
if old not in s: raise SystemExit('runtime log source anchor missing')
p.write_text(s.replace(old,new,1))

Path('safe_change_workflow_contract_test.go').write_text(r'''// SPDX-License-Identifier: MPL-2.0
package main
